...
AddEventHandler('esx_treasurehunter:GiveItem', function()
	...
	if Quantity >= 15 then
		...
		TriggerClientEvent('esx:showNotification', _source, 'Peças coletadas ~g~ segue caminho.')
	end
	...
	SetTimeout(Config.Zones.Vente.ItemTime, function()
		...
		if PlayersVente[source] == true then
			...
			if Quantity < Config.Zones.Vente.ItemRemove then
				TriggerClientEvent('esx:showNotification', _source, '~r~Não tens mais dinheiro para guardar.')
				...
				TriggerClientEvent('esx:showNotification', _source, 'Ganhaste ~g~$' .. Config.Zones.Vente.ItemPrice)
				Vente(_source)
			end
...
AddEventHandler('esx_treasurehunter:startVente', function()
	...
	if PlayersVente[_source] == false then
		TriggerClientEvent('esx:showNotification', _source, '~r~Não há dinheiro para guardar!')
		PlayersVente[_source] = false
	else
		PlayersVente[_source] = true
		TriggerClientEvent('esx:showNotification', _source, '~g~A guardar ~w~dinheiro...')
		Vente(_source)
	end
end)
